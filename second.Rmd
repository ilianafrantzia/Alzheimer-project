...
# Start with NA
dataset$TransmissionGroup <- NA

# Add all transmission groupings
dataset$TransmissionGroup[grepl("^A[0-9]$", dataset$TRANSMISSION)] <- "Automatic"
dataset$TransmissionGroup[grepl("^AM[0-9]$", dataset$TRANSMISSION)] <- "Automated Manual"
dataset$TransmissionGroup[grepl("^AS[0-9]$", dataset$TRANSMISSION)] <- "Select Shift Auto"
dataset$TransmissionGroup[grepl("^AV", dataset$TRANSMISSION)] <- "CVT"
dataset$TransmissionGroup[grepl("^M[0-9]$", dataset$TRANSMISSION)] <- "Manual"  # NEW

# Convert to factor
dataset$TransmissionGroup <- factor(dataset$TransmissionGroup)

# Check if any values still NA
table(dataset$TransmissionGroup, useNA = "ifany")

head(dataset)

sum(is.na(dataset$TransmissionGroup))

```{r pressure, echo=FALSE}
ggplot(dataset %>% filter(!is.na(TransmissionGroup)), aes(x = TransmissionGroup, y = CO2EMISSIONS, fill = TransmissionGroup)) +
  geom_boxplot() +
  theme_minimal()
...
model_multi <- lm(CO2EMISSIONS ~ ENGINESIZE + CYLINDERS + FUELCONSUMPTION_COMB + TransmissionType, data = data)
summary(model_multi)
...
model_final <- lm(CO2EMISSIONS ~ ENGINESIZE + CYLINDERS + FUELCONSUMPTION_COMB + TransmissionType, data = train)
