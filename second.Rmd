...
```{r}
pairs(regression[, c("ENGINESIZE", "CYLINDERS", "FUELCONSUMPTION_COMB", "CO2EMISSIONS")],
...
```{r scatter-cylinders}
ggplot(regression, aes(x = CYLINDERS, y = CO2EMISSIONS)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
```{r scatter-fuel-city}
ggplot(regression, aes(x = FUELCONSUMPTION_CITY, y = CO2EMISSIONS)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
```{r scatter-fuel-hwy}
ggplot(regression, aes(x = FUELCONSUMPTION_HWY, y = CO2EMISSIONS)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
# Scale numeric predictors (excluding CO2EMISSIONS)
fuel_scaled <- regression %>%
  mutate(across(
    .cols = c(ENGINESIZE, CYLINDERS, FUELCONSUMPTION_CITY, FUELCONSUMPTION_HWY, FUELCONSUMPTION_COMB, FUELCONSUMPTION_COMB_MPG),
    .fns = scale
  ))
...
```{r model-clean}
model_clean <- lm(CO2EMISSIONS ~ . - MODEL - MAKE -TransmissionGroup, data = train)
summary(model_clean)
```
